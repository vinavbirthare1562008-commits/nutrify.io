...
export default function MealCard({ meal, onEdit, onDelete, onDuplicate, onToggleFavorite, onAddToLog, isFavorite }) {
  return (
    <motion.article
      whileHover={{ y: -4 }}
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-[0.32em] text-slate-400">{meal.mealType} • {meal.foodCategory}</p>
          <div className="mt-3 flex items-center gap-3">
            ...
            <button
              ...
              className={`rounded-full px-3 py-1 text-sm transition ${
                isFavorite ? 'bg-rose-500/15 text-rose-300' : 'bg-white/5 text-slate-300 hover:bg-white/10'
              }`}
            >
      ...
      <div className="mt-5 grid grid-cols-3 gap-3 text-center text-sm text-slate-300">
        <div className="rounded-3xl bg-white/5 p-3">{meal.protein}g Protein</div>
        <div className="rounded-3xl bg-white/5 p-3">{meal.carbs}g Carbs</div>
        <div className="rounded-3xl bg-white/5 p-3">{meal.fats}g Fats</div>
      </div>
